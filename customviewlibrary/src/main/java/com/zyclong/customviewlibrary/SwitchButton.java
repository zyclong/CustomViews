...
public class SwitchButton extends CompoundButton {
    ...
    private  float PRESIZE = 0.55f; //按比例放大缩小开关
    private   int OFFSETX = 50; //宽度的偏移量
    ...
    private int [] mDefaultDrawablesID=new int[]{
//            R.drawable.switch_frame,
//            R.drawable.selector_switch_state,
...
//            R.drawable.selector_switch_slider
    ...
    private void init(AttributeSet attrs){
        ...
        if(attrs != null && getContext() != null){
            ...
            if(typedArray != null){
                ...
                PRESIZE=typedArray.getFloat(R.styleable.SwitchButton_zoom,0.55f);
                OFFSETX=typedArray.getInt(R.styleable.SwitchButton_offsetX,50);
                typedArray.recycle();
            }
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if(stateDrawable != null && stateMaskDrawable != null){
            ...
            stateBitmap=ReSize(stateBitmap,PRESIZE);//缩放处理
...
//                int src = canvas.saveLayer(0, 0, getWidth(), getHeight(), paint, Canvas.MATRIX_SAVE_FLAG | Canvas.CLIP_SAVE_FLAG | Canvas.HAS_ALPHA_LAYER_SAVE_FLAG | Canvas.FULL_COLOR_LAYER_SAVE_FLAG | Canvas.CLIP_TO_LAYER_SAVE_FLAG);
        ...
        if(sliderDrawable != null){
            ...
            if(sliderBitmap != null && !sliderBitmap.isRecycled()){
                sliderBitmap=ReSize(sliderBitmap,PRESIZE);
    ...
    private static Bitmap ReSize(Bitmap bitmap,float precentX,float precentY) {
        Matrix matrix = new Matrix();
        matrix.postScale(precentX,precentX); //长和宽放大缩小的比例
        Bitmap resizeBmp = Bitmap.createBitmap(bitmap,0,0,bitmap.getWidth(),bitmap.getHeight(),matrix,true);
        return resizeBmp;
    }
    ...
    public void setDrawables(Drawable frameBitmap, Drawable stateDrawable, Drawable stateMaskDrawable, Drawable sliderDrawable){
        ...
        offsetx=(int)(frameDrawable.getBounds().right*(1-PRESIZE)/2);
        offsety=(int)(frameDrawable.getBounds().bottom*(1-PRESIZE)/2);
        frameDrawable.setBounds(frameDrawable.getBounds().left,
                frameDrawable.getBounds().top,
                (int)( frameDrawable.getBounds().right*PRESIZE),
                (int)( frameDrawable.getBounds().bottom*PRESIZE)
        );
        stateMaskDrawable.setBounds(stateMaskDrawable.getBounds().left,
                stateMaskDrawable.getBounds().top,
                (int)( stateMaskDrawable.getBounds().right*PRESIZE),
                (int)( stateMaskDrawable.getBounds().bottom*PRESIZE)
        );

        this.tempMinSlideX = (-1 * (int)((stateDrawable.getIntrinsicWidth() - frameBitmap.getIntrinsicWidth())*PRESIZE));  //初始化X轴最小值
     ...
     * 设置图片
     */
    public void setDrawableResIds(){
        if(getResources() != null){
            setDrawables(
                    getResources().getDrawable(mDefaultDrawablesID[0]),
                    getResources().getDrawable(mDefaultDrawablesID[1]),
                    getResources().getDrawable(mDefaultDrawablesID[2]),
                    getResources().getDrawable(mDefaultDrawablesID[3])
            );
        }else {
            throw new IllegalArgumentException("ALL NULL");
        }
